...
;(function () {
  ...
  app.requireRole('passenger', {
    onAllow: async () => {
      await loadDriversList();
      await loadMyPassengerRequests();
      await prefillFromBoardCopy();
    ...
    return card(`
      <div>
        <div class="flex items-center gap-2 mb-1">
          ${badge('司机')}
          ${badge('可载 ' + (r.available_seats ?? '-') + ' 人')}
          ${badge(maskName(prof.full_name))}
          ${badge('性别 ' + genderLabel(prof.gender))}
          ${badge('行李上限 ' + (r.luggage_capacity ?? 0))}
...
async function loadDriversList() {
  ...
  let q = sb.from('driver_schedules')
    .select('*')
...
async function loadMyPassengerRequests() {
  ...
  const { data: { user } } = await sb.auth.getUser();
  if (!user) {
    wrap.innerHTML = '<div class="text-sm text-gray-500">登录后可查看你发布的需求</div>';
    counter.textContent = '—';
    return;
  }

  const { data, error } = await sb.from('passenger_requests').select('*')
    .eq('user_id', user.id)
  ...
  if (error) {
    wrap.innerHTML = `<div class="text-sm text-red-600">${error.message}</div>`;
    counter.textContent = '—';
    return;
  }
...
async function addPassengerRequest() {
  ...
  try { await window.__app.saveOrCreateProfile(); }
  catch (e) { toast('保存个人资料失败：' + e.message, 'error'); return; }

  const o = window.__app.readAddress('passenger-origin'); if (!o.ok) { toast(o.msg, 'error'); return; }
  const d = window.__app.readAddress('passenger-dest');   if (!d.ok) { toast(d.msg, 'error'); return; }
  ...
  const luggage= parseInt(document.getElementById('passenger-luggage')?.value, 10) || 0;
  const status = document.getElementById('passenger-status')?.value || 'open';
  ...
  btn.disabled = true; btn.textContent = editingPassengerId ? '保存中…' : '提交中…';
  ...
  try {
    ...
    await Promise.all([loadMyPassengerRequests(), loadDriversList()]);
    editingPassengerId = null; setPassengerSubmitLabel(false); resetPassengerForm();
  } catch (e) {
    toast('提交失败：' + e.message, 'error');
  } finally {
    btn.disabled = false; btn.textContent = '发布需求';
  }
}
...
function resetPassengerForm() {
  [
    ...
    'passenger-date','passenger-time','passenger-seats','passenger-note','passenger-luggage'
  ].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  const st = document.getElementById('passenger-status'); if (st) st.value = 'open';
...
document.addEventListener('click', async (e) => {
  ...
  if (act === 'edit-passenger') {
    ...
    const lugEl = document.getElementById('passenger-luggage'); if (lugEl) lugEl.value = (data.luggage_pcs ?? 0);
    const stEl  = document.getElementById('passenger-status');  if (stEl)  stEl.value  = (data.status ?? 'open');
...
async function prefillFromBoardCopy() {
  const usp = new URLSearchParams(location.search);
  const copyId = usp.get('copyFrom');
  if (!copyId) return;

  try {
    const { data, error } = await sb.from('passenger_requests')
      .select('*').eq('id', copyId).single();
    if (error || !data) return;

    // 把公告栏那条的出发/到达信息预填到表单；状态设为拼车中
    document.getElementById('passenger-origin-country').value = data.origin_country || 'US';
    document.getElementById('passenger-origin-street1').value = data.origin_street1 || '';
    document.getElementById('passenger-origin-street2').value = data.origin_street2 || '';
    document.getElementById('passenger-origin-city').value   = data.origin_city || '';
    ...
    // 默认 1 座，行李默认 0；状态设为拼车中
    const seatsEl = document.getElementById('passenger-seats');
    if (seatsEl) seatsEl.value = 1;

    const lugEl = document.getElementById('passenger-luggage');
    if (lugEl) lugEl.value = 0;

    const stEl = document.getElementById('passenger-status');
    if (stEl) stEl.value = 'carpooling';

    // 确保是“新增”模式
    if (typeof setPassengerSubmitLabel === 'function') setPassengerSubmitLabel(false);
    window.scrollTo({ top: document.getElementById('add-passenger-request').offsetTop - 200, behavior: 'smooth' });

    // 体验提示
    window.__app?.toast?.('已根据公告栏路线为你预填，可直接修改后发布拼单','info');
  } catch (e) {
    console.warn('prefillFromBoardCopy failed', e);
  }
}
